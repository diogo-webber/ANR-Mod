...
local function SerializeFX(inst, data)
    ...
    inst._rot = rot
    inst._rotrate = rotrate
    inst._alpha = alpha
    inst._alphadir = alphadir
    inst._alpharate = alpharate
end

local function DeserializeFX(inst, data)
    data.rot = inst._rot
    data.rotrate = (inst._rotrate - 1) * .5
    data.alpha = inst._alpha / 255
    data.alphadir = inst._alphadir
    data.alpharate = inst._alpharate / 63 * .02 + .005
end
...
local function OnUpdateFX(inst, data)
    ...
    SerializeFX(inst, data)

end

local function OnAnimDirty(inst)
    local anim = FXANIMS[inst._anim]
    if anim ~= nil then

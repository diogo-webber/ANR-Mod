...
local function PushMusic(inst)
    ...
    elseif GetPlayer():IsNear(inst, inst._playingmusic and 30 or 20) then
        inst._playingmusic = true
        GetPlayer():PushEvent("triggeredevent", { name = "dragonfly" })
    elseif inst._playingmusic and not GetPlayer():IsNear(inst, 40) then
        inst._playingmusic = false
    end
end

local function OnIsEngagedDirty(inst)
    if not inst._isengaged then
...
local function OnDragonflyEngaged(inst, data)
    ...
    if inst._isengaged ~= engaged then
        inst._isengaged = engaged
...
local function OnInit(inst)
    ...
    GetWorld():PushEvent("ms_registerlavapond", inst)
end

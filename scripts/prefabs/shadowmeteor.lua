...
--local SMASHABLE_TAGS = { "combat", "isinventoryitem" }
...
local function onexplode(inst)
    ...
    GetPlayer().components.playercontroller:ShakeCamera(inst, "FULL", shakeduration, shakespeed, shakescale, shakemaxdist)
    ...
    if not inst:IsOnValidGround() then
        ...
        for i, v in ipairs(ents) do
            ...
            if v:IsValid() and not v:IsInLimbo() then
                if v.components.workable ~= nil then
                    if v.components.workable.workleft > 0 and not (v.sg ~= nil and v.sg:HasStateTag("busy")) then
                        local work_action = v.components.workable.action
        ...
        for i, v in ipairs(inst.loot) do
            if math.random() <= v.chance then
                ...
                if canspawn then
                    local drop
                    if GetWorld().components.worldmeteorshower ~= nil then
                        drop = GetWorld().components.worldmeteorshower:SpawnMeteorLoot(v.prefab)
                    else

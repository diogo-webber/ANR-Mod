...
function _G.Launch(inst, launcher, basespeed)
    if inst ~= nil and inst.Physics ~= nil and inst.Physics:IsActive() and launcher ~= nil then
        ...
        local angle =
            spd > 0 and
            math.atan2(vz / spd, vx / spd) + (math.random() * 20 - 10) * _G.DEGREES or
            math.random() * 2 * PI
...
function _G.Launch2(inst, launcher, basespeed, speedmult, startheight, startradius, vertical_speed, force_angle)
    if inst ~= nil and inst.Physics ~= nil and inst.Physics:IsActive() and launcher ~= nil then
	    local x, y, z = launcher.Transform:GetWorldPosition()
		local x1, y1, z1 = inst.Transform:GetWorldPosition()
		local dx, dz = x1 - x, z1 - z
		local dsq = dx * dx + dz * dz
		local angle = force_angle ~= nil and (force_angle*_G.DEGREES) or nil
		if not angle then
			if dsq > 0 then
				local dist = math.sqrt(dsq)
				angle = math.atan2(dz / dist, dx / dist) + (math.random() * 20 - 10) * _G.DEGREES
			else
...
function _G.LaunchAt(inst, launcher, target, speedmult, startheight, startradius, randomangleoffset)
    if inst ~= nil and inst.Physics ~= nil and inst.Physics:IsActive() and launcher ~= nil then
        local x, y, z = launcher.Transform:GetWorldPosition()
        ...
        if target ~= nil then
            ...
            angle = (start_angle + (math.random() * angleoffset * 2) - target:GetAngleToPoint(x, 0, z)) * _G.DEGREES
        else
            local down = TheCamera:GetDownVec()
            angle = math.atan2(down.z, down.x) + (math.random() * angleoffset * 2 - angleoffset) * _G.DEGREES
        end
        local sina, cosa = math.sin(angle), math.cos(angle)
...
local COLLAPSIBLE_TAGS = { "combat", "pickable", "NPC_workable" }
for k, v in pairs(COLLAPSIBLE_WORK_ACTIONS) do
    table.insert(COLLAPSIBLE_TAGS, k.."_workable")
end
local NON_COLLAPSIBLE_TAGS = { "antlion", "groundspike", "flying", "shadow", "ghost", "playerghost", "FX", "NOCLICK", "DECOR", "INLIMBO" }

function _G.DestroyEntity(ent, destroyer, kill_all_creatures, remove_entity_as_fallback)
    if ent:IsValid() then
...
local TOSS_MUST_TAGS = { "isinventoryitem" }
local TOSS_CANT_TAGS = { "locomotor", "INLIMBO" }
function _G.LaunchAndClearArea(inst, radius, launch_basespeed, launch_speedmult, launch_startheight, launch_startradius)
    local x, y, z = inst.Transform:GetWorldPosition()

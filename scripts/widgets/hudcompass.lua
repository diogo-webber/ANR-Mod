...
local function TryCompass(self)
    if self.owner.components.inventory ~= nil then
        local equipment = self.owner.components.inventory:GetEquippedItem(EQUIPSLOTS.HANDS)
...
function HudCompass:OnUpdate(dt)
    ...
    -- Offsets from haunting
        ...
        spooky_offset = math.sin(t*0.005) * Lerp(self.currentcompass.spookyoffsettarget,0,spooky_t)
    end

    -- Offsets from sanity
    local sanity = self.owner.components.sanity
    local sanity_t = math.clamp(((sanity:GetPercent(true))) * 3, 0, 1) --math.clamp((sanity:IsInsanityMode() and sanity:GetPercentWithPenalty() or (1.0 - sanity:GetPercentWithPenalty())) * 3, 0, 1) --TODO
    local sanity_offset = math.sin(t*0.2) * Lerp(720, 0, sanity_t)

    -- Offset from full moon
    local fullmoon_t = GetClock():GetMoonPhase() == "full" and math.sin(GetClock():GetTimeLeftInEra() * math.pi) or 0
    local fullmoon_offset = math.sin(t*0.8) * Lerp(0, 720, fullmoon_t)

    -- Offset from wobble
    local wobble_offset = math.sin(t*2)*5

    self.offsetheading = EaseHeading(self.offsetheading, wobble_offset + fullmoon_offset + sanity_offset + spooky_offset, .5)

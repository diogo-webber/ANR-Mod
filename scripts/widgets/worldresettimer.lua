...
local WorldResetTimer = Class(Widget, function(self, owner)
    ...
    self.avatar = self.leftroot:AddChild(PlayerBadge(owner, owner.prefab or "", owner.playercolour or DEFAULT_PLAYER_COLOUR, 0))
    ...
    self.inst:ListenForEvent("showworldreset", function() self:StartTimer() end, GetWorld())
    self.inst:ListenForEvent("hideworldreset", function() self:StopTimer() end, GetWorld())
end)
...
function WorldResetTimer:RefreshButtons()
    ...
    if self.reset_text ~= nil then
        if not self.reset_text.shown then
...
function WorldResetTimer:StartTimer()
    ...
    if self._oncycleschanged == nil then
        self._oncycleschanged = function(world, cycles) self:UpdateCycles(GetClock():GetNumCycles()) end
        self.inst:ListenForEvent("daycomplete", self._oncycleschanged, GetWorld())
        self:UpdateCycles(GetClock():GetNumCycles())
    end
    if self._onworldresettick == nil then
        self._onworldresettick = function(world, data) self:UpdateCountdown(data.time) end
        self.inst:ListenForEvent("worldresettick", self._onworldresettick, GetWorld())
...
function WorldResetTimer:StopTimer()
    ...
    if self._oncycleschanged ~= nil then
        self.inst:RemoveEventCallback("daycomplete", self._oncycleschanged, GetWorld())
        self._oncycleschanged = nil
    end
    if self._onworldresettick ~= nil then
        self.inst:RemoveEventCallback("worldresettick", self._onworldresettick, GetWorld())
        self._onworldresettick = nil
    end
end
...
function WorldResetTimer:Reset()
    if self.started then
        GetWorld().components.worldreset:Reset()
    end
end

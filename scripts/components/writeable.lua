...
local function gettext(inst, viewer)
	if inst:HasTag("burnt") then
		return GetDescription(string.upper(viewer.prefab), inst, "BURNT")
	end
    ...
    return GetDescription(string.upper(viewer.prefab), inst, "UNWRITTEN")
end

local function onbuilt(inst, data)
    inst.components.writeable:BeginWriting(GetPlayer())
end
...
local Writeable = Class(function(self, inst)
    ...
    self:SetAutomaticDescriptionEnabled(true)
...
function Writeable:UpdateTexture()
...
function Writeable:SetAutomaticDescriptionEnabled(ad_enabled)
    ...
    if ad_enabled then
        ...
        self.inst.components.inspectable.description = nil
    end
end
...
function Writeable:Write(doer, text)
    ...
    if self.writer == doer and doer ~= nil and
        (text == nil or text:len() <= MAX_WRITEABLE_LENGTH) then
        self:SetText(text)
...
function Writeable:CollectSceneActions(doer, actions)
    if self.inst:HasTag("writeable") then
...
function Writeable:OnRemoveFromEntity()
    ...
    if self.inst.components.inspectable ~= nil and
        self.inst.components.inspectable.description == gettext then
        self.inst.components.inspectable.description = nil
    end
end

...
local TimeMultipliers = {
    ["evergreen"] = function()
        return TUNING.EVERGREEN_REGROWTH_TIME_MULT * ((GetSeasonManager():IsSummer() and 2) or (GetSeasonManager():IsWinter() and 0) or 1)
    end,
    ...
    ["deciduoustree"] = function()
        return TUNING.DECIDIOUS_REGROWTH_TIME_MULT * ((not GetSeasonManager():IsSpring() and 0) or 1)
    end,
    ["mushtree_tall"] = function()
        return TUNING.MUSHTREE_REGROWTH_TIME_MULT * ((not GetSeasonManager():IsWinter() and 0) or 1)
    end,
    ["mushtree_medium"] = function()
        return TUNING.MUSHTREE_REGROWTH_TIME_MULT * ((not GetSeasonManager():IsSummer() and 0) or 1)
    end,
    ["mushtree_small"] = function()
        return TUNING.MUSHTREE_REGROWTH_TIME_MULT * ((not GetSeasonManager():IsSpring() and 0) or 1)
    end,
    ["moon_tree"] = function()
        return TUNING.MOONTREE_REGROWTH_TIME_MULT * ((not GetSeasonManager():IsSpring() and 0) or 1)
    end,
    ["mushtree_moon"] = function()
        return TUNING.MOONMUSHTREE_REGROWTH_TIME_MULT * ((not GetSeasonManager():IsWinter() and 0) or 1)
    end,
}
...
local function RegisterUpdate(self)
    ...
    if UpdateBuckets == nil then
        ...
        UpdateTask = GetWorld():DoPeriodicTask(UPDATE_PERIOD, DoUpdate)
        self._bucket = { self }

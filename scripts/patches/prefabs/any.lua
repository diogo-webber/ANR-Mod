...
for i = 1, 45 do
    table.insert(HauntableLaunchAndSmash, "trinket_"..i)
end
...
local HauntableWorkAndIgnite = {
    "cave_banana_burnt",
    "livingtree",
    "marsh_tree",
}

local HauntableFreeze = {
    "eyeturret",
}
...
local function onhauntorange(inst)
    if math.random() <= TUNING.HAUNT_CHANCE_OCCASIONAL then
        ...
        if target ~= nil then
            ...
            local offset = FindWalkableOffset(pos, start_angle, math.random(8, 12), 16, false, true)
            if offset ~= nil then
...
return function(inst)
    ...
    if table.contains(HauntableLaunchAndSmash, tostring(inst.prefab)) then
        MakeHauntableLaunchAndSmash(inst)
    end
    
    if table.contains(HauntableWork, tostring(inst.prefab)) then
        MakeHauntableWork(inst)
    end
    
    if table.contains(HauntableWorkAndIgnite, tostring(inst.prefab)) then
        MakeHauntableWorkAndIgnite(inst)
    end
    
    if table.contains(HauntableFreeze, tostring(inst.prefab)) then
        MakeHauntableFreeze(inst)
    end
    
    if table.contains(HauntableIgnite, tostring(inst.prefab)) then
        MakeHauntableIgnite(inst)
    end
    
    if table.contains(HauntableLaunchAndIgnite, tostring(inst.prefab)) then
        MakeHauntableLaunchAndIgnite(inst)
    end

    if table.contains(HauntableLaunchOrChangePrefab, tostring(inst.prefab)) then
        MakeHauntableLaunchOrChangePrefab(inst)
    end
    
    if table.contains(HauntablePerish, tostring(inst.prefab)) then
        MakeHauntablePerish(inst)
    end    
    
    if table.contains(HauntableLaunchAndPerish, tostring(inst.prefab)) then
        MakeHauntableLaunchAndPerish(inst)
    end
    
    if table.contains(HauntablePanic, tostring(inst.prefab)) then
        MakeHauntablePanic(inst)
    end
    
    if table.contains(HauntablePanicAndIgnite, tostring(inst.prefab)) then
        MakeHauntablePanicAndIgnite(inst)
    end
    
    if table.contains(HauntablePlayAnim, tostring(inst.prefab)) then
        MakeHauntablePlayAnim(inst)
    end

    if table.contains(HauntableDropFirstItem, tostring(inst.prefab)) then
        MakeHauntableDropFirstItem(inst)
    end
    
    if table.contains(HauntableLaunchAndDropFirstItem, tostring(inst.prefab)) then
        MakeHauntableLaunchAndDropFirstItem(inst)
    end
    
    if table.contains(HauntableDropItemOrWork, tostring(inst.prefab)) then
    ...
    if inst.prefab == "flower" then
        MakeHauntableChangePrefab(inst, "flower_evil")
    end
    
    if inst.prefab == "hound" or inst.prefab == "icehound" or inst.prefab == "firehound" then
    ...
    if inst.prefab == "red_cap" or inst.prefab == "red_cap_cooked" or inst.prefab == "green_cap" or 
        inst.prefab == "green_cap_cooked" or inst.prefab == "blue_cap" or inst.prefab == "blue_cap_cooked" then
        MakeHauntableLaunchAndPerish(inst)
    ...
    if inst.prefab == "petals" then
        MakeHauntableLaunchAndPerish(inst)
        AddHauntableCustomReaction(inst, OnHauntFlower, false, true, false)
    end

    if inst.prefab == "petals_evil" then
        MakeHauntableLaunchAndPerish(inst)
        inst:ListenForEvent("spawnedfromhaunt", OnSpawnedFromHauntCap)
    end

    if inst.prefab == "killerbee" then
        MakeHauntablePanic(inst)
        inst:ListenForEvent("spawnedfromhaunt", OnSpawnedFromHauntKillerBee)
    end    

    if inst.prefab == "beefalo" or inst.prefab == "monkey" or inst.prefab == "slurtle" or inst.prefab == "snurtle" or inst.prefab == "spat" then
        MakeHauntablePanic(inst)
        AddHauntableCustomReaction(inst, CustomOnHauntBeefalo, true, false, true)    
    end

    if inst.prefab == "mosquito" then
        MakeHauntablePanic(inst)
        AddHauntableCustomReaction(inst, function(inst, haunter)
            if math.random() <= TUNING.HAUNT_CHANCE_OCCASIONAL then
    ...
    if inst.prefab == "greenstaff" then
    ...
    if inst.prefab == "yellowstaff" then
        AddHauntableCustomReaction(inst, onhauntlight, true, false, true)
    end

    if inst.prefab == "tentacle_pillar_arm" then
    ...
    if inst.prefab == "balloons_empty" then
        if not inst.components.hauntable then inst:AddComponent("hauntable") end
        inst.components.hauntable:SetHauntValue(TUNING.HAUNT_TINY)
        inst.components.hauntable:SetOnHauntFn(function()
    ...
    if inst.prefab == "beehive" or inst.prefab == "wasphive" then
    ...
    if inst.prefab == "crow" or inst.prefab == "robin" or inst.prefab == "robin_winter" then
        if not inst.components.hauntable then inst:AddComponent("hauntable") end
        inst.components.hauntable:SetHauntValue(TUNING.HAUNT_TINY)
		inst.components.hauntable.panicable = true
    end
    ...
    if inst.prefab == "carrot_planted" or inst.prefab == "cookpot" or inst.prefab == "fireflies" then
        if not inst.components.hauntable then inst:AddComponent("hauntable") end
        inst.components.hauntable:SetHauntValue(TUNING.HAUNT_TINY)
    end
    ...
    if inst.prefab == "slow_farmplot" or inst.prefab == "fast_farmplot" then
        ...
        inst.components.hauntable:SetOnHauntFn(function() 
            if inst.components.grower ~= nil and
                ...
                local fert_percent = inst.components.grower.cycles_left / inst.components.grower.max_cycles_left
    ...
    if inst.prefab == "houndmound" then
    ...
    if inst.prefab == "researchlab" or inst.prefab == "researchlab2" or inst.prefab == "researchlab3" or inst.prefab == "researchlab4" then
        if not inst.components.hauntable then inst:AddComponent("hauntable") end
        inst.components.hauntable:SetHauntValue(TUNING.HAUNT_TINY)
    end

    if inst.prefab == "mermhouse" then
    ...
    if inst.prefab == "molebathill" then
        if not inst.components.hauntable then inst:AddComponent("hauntable") end
        inst.components.hauntable:SetHauntValue(TUNING.HAUNT_SMALL)
        inst.components.hauntable:SetOnHauntFn(function()
    ...
    if inst.prefab == "monkeybarrel" then
    ...
    if inst.prefab == "onemanband" then
        if not inst.components.hauntable then inst:AddComponent("hauntable") end
        inst.components.hauntable:SetHauntValue(TUNING.HAUNT_SMALL)
        inst.components.hauntable:SetOnHauntFn(function(inst, haunter)
    ...
    if inst.prefab == "plant_normal" then
        if not inst.components.hauntable then inst:AddComponent("hauntable") end
        inst.components.hauntable:SetHauntValue(TUNING.HAUNT_TINY)
        inst.components.hauntable:SetOnHauntFn(function(inst, haunter)
    ...
    if inst.prefab == "pond" or inst.prefab == "pond_mos" or inst.prefab == "pond_cave" then
        if not inst.components.hauntable then inst:AddComponent("hauntable") end
        inst.components.hauntable:SetHauntValue(TUNING.HAUNT_TINY)
    end

    if inst.prefab == "rabbithole" then
        if not inst.components.hauntable then inst:AddComponent("hauntable") end
        inst.components.hauntable:SetHauntValue(TUNING.HAUNT_SMALL)
        inst.components.hauntable:SetOnHauntFn(function(inst, haunter)
            return not inst.spring
    ...
    if inst.prefab == "spiderden" or inst.prefab == "spiderden_2" or inst.prefab == "spiderden_3" then
    ...
    if inst.prefab == "stafflight" then
        ...
        inst.components.hauntable:SetOnHauntFn(function(inst, haunter)
            if inst.components.timer:TimerExists("extinguish") then
                ...
                inst:ListenForEvent("animover", function() inst.SoundEmitter:KillSound("staff_star_loop") end)
    ...
    if inst.prefab == "pighead" or inst.prefab == "mermhead" then
        if not inst.components.hauntable then inst:AddComponent("hauntable") end
        inst.components.hauntable.cooldown = TUNING.HAUNT_COOLDOWN_MEDIUM
        inst.components.hauntable:SetOnHauntFn(function(inst, haunter)

            local function OnFinishHaunt(inst)
                if inst.awake and not (GetClock() and GetClock():GetMoonPhase() == "full" and GetClock():IsNight() or inst:HasTag("burnt")) then
                    inst.awake = nil
    ...
    if inst.prefab == "tallbirdnest" then
        if not inst.components.hauntable then inst:AddComponent("hauntable") end
        inst.components.hauntable:SetHauntValue(TUNING.HAUNT_TINY)
    end

    if inst.prefab == "gemsocket" then
        if not inst.components.hauntable then inst:AddComponent("hauntable") end
    ...
    if inst.prefab == "trap_teeth" or inst.prefab == "trap_bramble" then
        if not inst.components.hauntable then inst:AddComponent("hauntable") end
        inst.components.hauntable:SetOnHauntFn(function()
            if inst.components.mine == nil or inst.components.mine.inactive then
                inst.components.hauntable.hauntvalue = TUNING.HAUNT_TINY
    ...
    if inst.prefab == "treasurechest" or inst.prefab == "pandoraschest" or inst.prefab == "minotaurchest" then
        if not inst.components.hauntable then inst:AddComponent("hauntable") end
        inst.components.hauntable:SetHauntValue(TUNING.HAUNT_TINY)
    end
